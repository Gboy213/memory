...
Лог: $DRIFT_MARKERS_LOG или ~/.claude/drift-markers.tsv. Никогда не падает
(всегда exit 0) и ничего не печатает в stdout — не ломает сессию и не засоряет контекст.
"""
import sys, json, re, os, datetime

LOG = os.environ.get("DRIFT_MARKERS_LOG") or os.path.join(os.path.expanduser("~"), ".claude", "drift-markers.tsv")
